...
for year in range(2020,2022):
    ...
    osszeshalott =  dfkshdiff['diff összesen'].sum()
    ...
    dfkshdiff['Valós COVID'] = [0] * len(dfkshdiff)
    dfkshdiff['Járványkezelés'] = [0] * len(dfkshdiff)

    for k in KATEGORIA:
        dfkshdiff['covid ' + k] = dfkshdiff['covid ' + k].astype(int)
        dfkshdiff['valoscovid ' + k] = dfkshdiff.apply(lambda row: min(row['diff ' + k], row['covid ' + k]), axis=1)
    ...
    for k in KATEGORIA:
        dfkshdiff['Valós COVID'] = dfkshdiff['Valós COVID'] + dfkshdiff['valoscovid ' + k]
        dfkshdiff['Járványkezelés'] = dfkshdiff['Járványkezelés'] + dfkshdiff['jarvany ' + k]



    covid_halott = dfkshdiff['Valós COVID'].sum()
    jarvany_halott = osszeshalott - covid_halott

    dfkshdiff['Dátum'] = dfkshdiff['A hét kezdő napja'] 

    maxjarvany = (int((max(dfkshdiff['Járványkezelés']) + 99) / 100) * 100)
    maxcovid = (int((max(dfkshdiff['Valós COVID']) + 99) / 100) * 100)
    maxmind = maxjarvany + maxcovid
...
fig.savefig(BASEDIR + "/COVIDésJárványkezelésElhunytak.png")
